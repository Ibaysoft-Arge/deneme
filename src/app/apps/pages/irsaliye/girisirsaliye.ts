...
@Component({
    templateUrl: './girisIrsaliye.html',
    animations: [toggleAnimation, slideDownUp]
})
export class GirisIrsaliyeComponent implements OnInit, AfterViewInit {
    ...
    editingGirisIrsaliyeId: string | null = null;
    ...
    searchItems(term: string, type: string): Observable<any[]> {
        if (type === 'SKU') {
            return this.skuService.searchSkusSatis(term).pipe(
                catchError(() => of([]))
            );
        } else if (type === 'irsaliye') {
            return this.irsaliyeService.searchIrsaliyeGiris(term).pipe(
                catchError(() => of([]))
            );
        }
        return of([]);
    ...
    selectIrsaliye(selectedItem: any, modal: any): void {
        ...
        this.isEditing = true;
        this.editingGirisIrsaliyeId = selectedItem._id;
        this.modalSearchTermIrsaliye = "";
        modal.close();
    }
    ...
    onSubmitSku(): void {
        ...
        if (this.editingGirisIrsaliyeId) {
            // Mevcut irsaliye -> update sku
            this.updateGirisIrsaliyeSku(this.editingGirisIrsaliyeId, this.skus);
        } else {
    ...
    addIrsaliye(irsaliyeData: any): void {
        this.irsaliyeService.addGirisIrsaliye(irsaliyeData).subscribe({
            next: (addedIrsaliye) => {
    ...
    updateIrsaliye(id: string, updatedIrsaliye: any): void {
        this.irsaliyeService.updateGirisIrsaliye(id, updatedIrsaliye).subscribe({
            next: () => {
    ...
    updateGirisIrsaliyeSku(id: string, updatedIrsaliyeSkus: any[]): void {
        this.irsaliyeService.updateGirisIrsaliyeSku(id, updatedIrsaliyeSkus).subscribe({
            next: () => {
                this.notificationService.showNotification(this.translate.instant("updateSuccess"), 'success', 'top-end');
            },
            error: (error) => {
    ...
    deleteSku(sku: any) {
        if (confirm(this.translate.instant("deleteConfirmation"))) {
            ...
            this.irsaliyeService.deleteGirisIrsaliyeSku(sku._id).subscribe({
                next: () => {
